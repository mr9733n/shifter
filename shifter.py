...
def write_to_debug_log(log_path, messages):
    """Writes log messages to debug file"""
    with open(log_path, "a") as f:
        for message in messages:
            f.write(f"{datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')} {message}\n")
...
def run_copy_job():
    """Schedule the copy job to run at a specific time"""
    config = read_config()
    ...
    run_once = config.get("run_once", False)  # Get the "run_once" parameter from config, default to False
    ...
    if run_once:
        # Run the copy job once
        copy_files_job()
        sys.exit()  # Exit the script after execution
    else:
        while True:
